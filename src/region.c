...
void region_init(GameState *gs)
{
    ...
    /* --- The North (Scotland, northern England, and parts of Scandinavia (cold climate, clans, castles).) --- */
    R(0).name               = "The North";
    ...
    /* --- Dorne (Spain ;especially Andalusia and Moorish Iberia, with some Middle Eastern influence.) --- */
    R(1).name               = "Dorne";
    ...
    /* --- The Vale (Alpine regions such as Switzerland and Austria (mountains and isolated valleys).) --- */
    R(3).name               = "The Vale";
    R(3).population         = 0.15f;  /* small Alpine nations, mountains limit settlement */
    R(3).infected           = 0.00f;
    R(3).vaccinated         = 0.00f;
    R(3).healthcareCapacity = 0.90f;  /* Switzerland = world's best healthcare system    */
    ...
    /* --- The Iron Islands (Australia/Oceania — isolated.Vikings from Scandinavia and the Norse-Gaels of the British Isles.) --- */
...
void region_update_states(GameState *gs)
{
    for (int i = 0; i < MAX_REGIONS; i++)
    {
        ...
        if      (inf < 0.05f) gs->regions[i].state = REGION_CLEAN;
        else if (inf < 0.30f) gs->regions[i].state = REGION_INFECTED;
        else if (inf < 0.60f) gs->regions[i].state = REGION_CRITICAL;
